...
class ProductForm(forms.Form):
    ...
    veg_type_choices = [
        ...
        ('cosmos', 'Cosmos'),
        ('foxglove', 'Foxglove'),
        # add/remove other groups as needed
    ]
    ...
    def __init__(self, *args, seed_availabilities=None, **kwargs):
        ...
        # Dynamically load store choices at runtime
        store_choices = [('1', 'All Stores')] + [(str(store.store_number), store.name) for store in Store.objects.all()]
        self.fields['store_selection'].choices = store_choices
        ...
        # Sort by category, veg_type, and variety
...
#     store_choices = [('1', 'All Stores')]  # static part

#     store_field = forms.ChoiceField(choices=[])  # define field without choices yet

#     def __init__(self, *args, **kwargs):
#         super().__init__(*args, **kwargs)
#         dynamic_choices = [(store.store_number, store.name) for store in Store.objects.all()]
#         self.fields['store_field'].choices = self.store_choices + dynamic_choices
